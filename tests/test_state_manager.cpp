...
public:
    ...
    bool operator==(const StringState& other) const { return value == other.value; }
...
public:
    ...
    bool operator==(const FloatState& other) const { return value == other.value; }
...
TEST_F(StateManagerTest, StateObjDataTypes_float) {
  ...
  auto [error_float, read_float] = state_manager->read("test_float", template_float);
...
TEST_F(StateManagerTest, StateObjDataTypes_bool) {
  ...
  auto [error_bool, read_bool] = state_manager->read("test_bool", template_bool);
...
TEST_F(StateManagerTest, StateObjDataTypes_array) {
  ...
  auto [error_array, read_array] = state_manager->read("test_array", ArrayState{});
...
TEST_F(StateManagerTest, StateObjDataTypes_object) {
  ...
  auto [error_object, read_object] = state_manager->read("test_object", ObjectState{});
...
TEST_F(StateManagerTest, StateObjDataTypes_custom_class_basic) {
  ...
  auto [error_read, read_obj] = state_manager->read("test_user_profile", UserProfile{});
...
TEST_F(StateManagerTest, StateObjDataTypes_custom_class_roundtrip) {
  ...
  auto [error1, data1] = state_manager->read("test_roundtrip_user", UserProfile{});
  ...
  auto [error2, data2] = state_manager->read("test_roundtrip_user2", UserProfile{});
...
TEST_F(StateManagerTest, CRUDOperations_basic) {
  ...
  auto [error_after_delete, deleted_value] = state_manager->read(key, ObjectState{});
...
TEST_F(StateManagerTest, ErrorHandling_nonexistent_key) {
  ...
  auto [error_nonexistent, value_nonexistent] = state_manager->read("nonexistent_key", StringState{});
...
TEST_F(StateManagerTest, EdgeCases_empty_string) {
  ...
  auto [error_empty, read_empty] = state_manager->read("empty_string", StringState{});
...
TEST_F(StateManagerTest, EdgeCases_special_characters) {
  ...
  StringState special_chars("Special chars: üñíçödé 中文 🚀 \"quotes\" \\backslash");
  EXPECT_TRUE(state_manager->write("special_chars", special_chars));
  
  auto [error_special, read_special] = state_manager->read("special_chars", StringState{});
  ...
  EXPECT_NE(string_state, nullptr);
  EXPECT_EQ(string_state->value, "Special chars: üñíçödé 中文 🚀 \"quotes\" \\backslash");
}
