...
public:
    ...
    UserEvent() = default;
    UserEvent(const std::string& event_type, int user_id, const std::string& username, 
...
STATE_OBJ_DEFINE_TYPE(UserEvent, event_type, user_id, username, timestamp, ip_address, session_duration, changes)
...
public:
    ...
    OrderMessage(const std::string& action, int order_id, int user_id = 0, int items = 0)
...
STATE_OBJ_DEFINE_TYPE(OrderMessage, action, order_id, user_id, items, tracking, delivery_time)
...
public:
    ...
    PaymentMessage(int order_id, double amount, const std::string& status, const std::string& method = "")
...
STATE_OBJ_DEFINE_TYPE(PaymentMessage, order_id, amount, status, method, transaction_id)
...
public:
    ...
    BroadcastMessage(int message_id, const std::string& message, const std::string& priority = "normal")
...
STATE_OBJ_DEFINE_TYPE(BroadcastMessage, message_id, message, timestamp, priority)
...
public:
    ...
    StateChangeNotification(const std::string& operation, const std::string& key, const std::string& timestamp)
...
STATE_OBJ_DEFINE_TYPE(StateChangeNotification, operation, key, timestamp, old_value, new_value)
...
public:
    ...
    UserData() = default;
    UserData(int id, const std::string& name, const std::string& email, const std::string& status)
...
STATE_OBJ_DEFINE_TYPE(UserData, id, name, email, status, last_login, login_count)
...
void demonstrateBasicPubSub() {
    ...
    subscriber.subscribe(channel, [&](std::unique_ptr<StateManager::StateObj> message) {
        ...
        if (userEvent) {
            std::cout << "📨 Received user event: " << userEvent->event_type << std::endl;
            ...
            if (!userEvent->ip_address.empty()) {
                std::cout << "   IP Address: " << userEvent->ip_address << std::endl;
            }
            if (userEvent->session_duration > 0) {
                std::cout << "   Session Duration: " << userEvent->session_duration << " seconds" << std::endl;
            }
...
void demonstrateMultipleChannels() {
    ...
    // Orders channel
    stateManager.subscribe("orders", [&](std::unique_ptr<StateManager::StateObj> message) {
        std::lock_guard<std::mutex> lock(outputMutex);
        ordersReceived.fetch_add(1);
        OrderMessage* order = dynamic_cast<OrderMessage*>(message.get());
        if (order) {
            std::cout << "🛒 ORDER: " << order->action << " - Order #" << order->order_id << std::endl;
        }
    }, OrderMessage{});
    
    // Payments channel
    stateManager.subscribe("payments", [&](std::unique_ptr<StateManager::StateObj> message) {
        std::lock_guard<std::mutex> lock(outputMutex);
        paymentsReceived.fetch_add(1);
        PaymentMessage* payment = dynamic_cast<PaymentMessage*>(message.get());
        if (payment) {
            std::cout << "💳 PAYMENT: $" << payment->amount << " - " << payment->status << std::endl;
        }
    }, PaymentMessage{});
    
    // Notifications channel
    stateManager.subscribe("notifications", [&](std::unique_ptr<StateManager::StateObj> message) {
        std::lock_guard<std::mutex> lock(outputMutex);
        notificationsReceived.fetch_add(1);
        NotificationMessage* notification = dynamic_cast<NotificationMessage*>(message.get());
        if (notification) {
            std::cout << "🔔 NOTIFICATION: " << notification->message << std::endl;
        }
    }, NotificationMessage{});
    ...
    std::vector<std::pair<std::string, std::unique_ptr<StateManager::StateObj>>> channelMessages;
    
    // Create order
    channelMessages.emplace_back("orders", std::make_unique<OrderMessage>("created", 12345, 1001, 3));
    channelMessages.emplace_back("notifications", std::make_unique<NotificationMessage>("order_confirmation", "Your order #12345 has been confirmed"));
    
    // Process payment
    auto payment1 = std::make_unique<PaymentMessage>(12345, 99.99, "processing", "credit_card");
    ...
    channelMessages.emplace_back("notifications", std::make_unique<NotificationMessage>("shipping", "Your order #12345 has been shipped. Tracking: TRK123456789"));
    ...
    channelMessages.emplace_back("notifications", std::make_unique<NotificationMessage>("delivery", "Your order #12345 has been delivered"));
...
void demonstrateMultipleSubscribers() {
    ...
    for (int i = 0; i < numSubscribers; ++i) {
        ...
        subscribers[i]->subscribe(channel, [&, i](std::unique_ptr<StateManager::StateObj> message) {
            std::lock_guard<std::mutex> lock(outputMutex);
            messageCounters[i].fetch_add(1);
            BroadcastMessage* broadcast = dynamic_cast<BroadcastMessage*>(message.get());
            if (broadcast) {
                std::cout << "Subscriber " << i << " received: " << broadcast->message << std::endl;
            }
        }, BroadcastMessage{});
    }
    ...
    for (int msgId = 1; msgId <= 5; ++msgId) {
        BroadcastMessage message(msgId, "Broadcast message #" + std::to_string(msgId), 
...
void demonstrateStateChangeNotifications() {
    ...
    notificationManager.subscribe(notificationChannel, [&](std::unique_ptr<StateManager::StateObj> notification) {
        std::lock_guard<std::mutex> lock(outputMutex);
        notificationsReceived.fetch_add(1);
        
        StateChangeNotification* stateChange = dynamic_cast<StateChangeNotification*>(notification.get());
        if (stateChange) {
            ...
            if (!stateChange->old_value.is_null()) {
                std::cout << "   Old Value: " << stateChange->old_value.dump() << std::endl;
            }
            if (!stateChange->new_value.is_null()) {
                std::cout << "   New Value: " << stateChange->new_value.dump() << std::endl;
            }
    ...
    auto publishStateChange = [&](const std::string& operation, const std::string& key, 
        ...
        StateChangeNotification notification(operation, key, 
            "2024-08-05T" + std::to_string(std::chrono::duration_cast<std::chrono::seconds>(
    ...
    auto [deleteReadError, dataToDeleteObj] = dataManager.read(userKey, UserData{});
...
int main() {
    std::cout << "StateManager Library - Pub/Sub Demo (StateObj Interface)" << std::endl;
    std::cout << "=========================================================" << std::endl;
    std::cout << "This demo showcases the publish/subscribe functionality with StateObj." << std::endl;
    ...
    try {
        ...
        std::cout << "functionality with type-safe StateObj messaging for real-time" << std::endl;
