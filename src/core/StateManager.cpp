  ...
  try {
    ...
    if (success) {
      ...
      this->log(logging::LogLevel::ERROR,
                "Failed to write value to key: " + key);
    }

    return success;
  } catch (const json::exception &e) {
    this->log(logging::LogLevel::ERROR,
              "JSON serialization error for key " + key + ": " + e.what());
    return false;
...
std::pair<error, std::unique_ptr<StateObj>>
StateManager::read(const std::string &key, const StateObj &template_obj) {
  try {
    RedisStorage storage;
    ...
    this->log(logging::LogLevel::ERROR,
              "JSON deserialization error for key " + key + ": " + e.what());
...
void StateManager::subscribe(
    ...
    std::function<void(std::unique_ptr<StateObj>)> callback,
    const StateObj &template_obj) {
  try {
    ...
    // Wrap the user callback to handle JSON deserialization and StateObj
    // creation
    auto wrapped_callback = [this, callback = std::move(callback), channel,
                             template_ptr = std::shared_ptr<StateObj>(std::move(
                                 template_clone))](const std::string &message) {
      ...
      this->log(logging::LogLevel::DEBUG,
                "Successfully processed message on channel: " + channel);
    };
...
bool StateManager::publish(const std::string &channel, const StateObj &data) {
  try {
    ...
    if (success) {
      this->log(logging::LogLevel::INFO,
                "Successfully published to channel: " + channel);
    } else {
      this->log(logging::LogLevel::ERROR,
    ...
    this->log(logging::LogLevel::ERROR,
              "JSON serialization error for channel " + channel + ": " +
                  e.what());
    return false;
  } catch (const std::exception &e) {
    this->log(logging::LogLevel::ERROR,
