...
private:
  ...
  std::function<bool(const uint queueSize, const uint workerCount)> scaleUp =
      [](uint queueSize, uint workerCount) {
        return (queueSize > 10 * workerCount && workerCount < 10);
      };
  std::function<bool(const uint queueSize, const uint workerCount)> scaleDown =
      [](uint queueSize, uint workerCount) {
...
public:
  ThreadPool();
  ~ThreadPool();
  void addTask(std::function<void(const std::string &)> task,
               std::string payload);
  void setScaleUp(
